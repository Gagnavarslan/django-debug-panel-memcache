...
import pylibmc
import memcache
import debug_toolbar_memcache.panels.memcache
import debug_toolbar_memcache.panels.pylibmc
...
class TestCase(unittest.TestCase):
    ...
    def assertLastCall(self, expected_function, expected_args, base_message):
        self.assertCall(debug_toolbar_memcache.panels.instance.last(),
                expected_function, expected_args, base_message)

    def assertLastCallRaised(self, expected_function, expected_args,
            base_message):
        call = debug_toolbar_memcache.panels.instance.last()
...
class TestPyLibMc(TestCase):
    ...
    def test_basic(self):
        debug_toolbar_memcache.panels.instance.reset()
        ...
        n = debug_toolbar_memcache.panels.instance.size()
        self.assertTrue(client.delete_multi(multi_keys), 'delete_multi')
        calls = debug_toolbar_memcache.panels.instance.calls()
        ...
        self.assertEquals(26, debug_toolbar_memcache.panels.instance.size(),
                'total number of calls')

        # test out the panel, mainly resetting
        panel = debug_toolbar_memcache.panels.memcache.MemcachePanel()
...
class TestMemcache(TestCase):
    ...
    def test_basic(self):
        ...
        self.assertEquals(24, debug_toolbar_memcache.panels.instance.size(),
                'total number of calls')
